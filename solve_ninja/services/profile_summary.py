...
def generate_profile_summary(user: str) -> str:
    """
    ...
    actions_text = str(portfolio["actions"])
    total_hours = portfolio.get("total_hours_invested", 0)
    total_actions = portfolio.get("total_actions", 0)
    ...
    messages = [
        {"role": "system", "content": PROFILE_SUMMARY_SYSTEM_PROMPT},
        {
            "role": "user",
            "content": (
                f"user name: {portfolio.get('first_name', '')}\n"
                f"total hours invested: {total_hours}\n"
                f"total number of actions: {total_actions}\n"
                f"today's date: {datetime.now().strftime('%Y-%m-%d')}\n"
                f"all_actions: {actions_text}"
            ),
        },
    ]
...
def update_user_summary_in_metadata(user: str, summary: str) -> None:
    """Update User Metadata summary field and ai_summary_last_updated timestamp."""
    if not user:
        return
    ...
    if frappe.db.exists("User Metadata", user):
        frappe.db.set_value(
            "User Metadata", user,
            {"summary": summary or "", "ai_summary_last_updated": now}
        )
        frappe.db.commit()
    else:
        frappe.get_doc({
